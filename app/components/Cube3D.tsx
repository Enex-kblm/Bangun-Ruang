...
export function Cube3D({ size, opacity, toggles, colors }: Cube3DProps) {
  const meshRef = useRef<THREE.Mesh>(null);
  const halfSize = size / 2;

  const vertices = [
    [-halfSize, -halfSize,  halfSize], // A - 0
    [ halfSize, -halfSize,  halfSize], // B - 1
    [ halfSize,  halfSize,  halfSize], // C - 2
    [-halfSize,  halfSize,  halfSize], // D - 3
    [-halfSize, -halfSize, -halfSize], // E - 4
    [ halfSize, -halfSize, -halfSize], // F - 5
    [ halfSize,  halfSize, -halfSize], // G - 6
    [-halfSize,  halfSize, -halfSize], // H - 7
  ];

  // Define edges for thick lines
  const edges = [
    // Bottom face
    [[-halfSize, -halfSize, halfSize], [halfSize, -halfSize, halfSize]], // A-B
    [[halfSize, -halfSize, halfSize], [halfSize, -halfSize, -halfSize]], // B-F
    [[halfSize, -halfSize, -halfSize], [-halfSize, -halfSize, -halfSize]], // F-E
    [[-halfSize, -halfSize, -halfSize], [-halfSize, -halfSize, halfSize]], // E-A
    
    // Top face
    [[-halfSize, halfSize, halfSize], [halfSize, halfSize, halfSize]], // D-C
    [[halfSize, halfSize, halfSize], [halfSize, halfSize, -halfSize]], // C-G
    [[halfSize, halfSize, -halfSize], [-halfSize, halfSize, -halfSize]], // G-H
    [[-halfSize, halfSize, -halfSize], [-halfSize, halfSize, halfSize]], // H-D
    
    // Vertical edges
    [[-halfSize, -halfSize, halfSize], [-halfSize, halfSize, halfSize]], // A-D
    [[halfSize, -halfSize, halfSize], [halfSize, halfSize, halfSize]], // B-C
    [[halfSize, -halfSize, -halfSize], [halfSize, halfSize, -halfSize]], // F-G
    [[-halfSize, -halfSize, -halfSize], [-halfSize, halfSize, -halfSize]], // E-H
  ];
  ...
  // Define all diagonal planes for cube
  ...
  // Fungsi untuk membuat garis arsiran yang tebal dan jelas
  const createHatchLines = (planeVertices: number[], steps: number = 300) => {
    ...
    // Vektor arah untuk arsiran (dari A ke H dan B ke G)
    const side1 = new THREE.Vector3().subVectors(v3, v0); // A ke H
    const side2 = new THREE.Vector3().subVectors(v2, v1); // B ke G
    
    for (let i = 0; i <= steps; i++) {
      const t = i / steps;
      
      // Titik start dari garis A-H
      const start = new THREE.Vector3().addVectors(
        v0,
        side1.clone().multiplyScalar(t)
      );
      
      // Titik end dari garis B-G  
